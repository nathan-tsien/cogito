...
//! and spec §6.3.
//!
//! Lives in `cogito-protocol` (and not in `cogito-skills`) so the Brain
//! layer (`cogito-core::harness`, in particular H06 stream-demux) can call
//! it without crossing ADR-0004's "Brain may only import cogito-protocol"
//! boundary. The functions are pure (no I/O); they operate on `&str` and
//! a small streaming `FenceState`.
