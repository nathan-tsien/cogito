...
- **`cogito-protocol`** 新增类型(全部 additive,不触发 `SCHEMA_VERSION` bump,符合 ADR-0007 §"forward-compat"):
  ...
  - **`ModelGateway::model_limits() -> ModelLimits`** 方法(additive,带保守默认实现);`ModelLimits { model_id, context_window_tokens, max_output_tokens }` 类型——使 Compactor 阈值跨 provider 自适应,详 §6.7 / §10
  - `TokenThreshold` enum(`Ratio` / `Absolute`)作为 `TruncateConfig.max_tokens` 字段类型,详 §10.1
- **`cogito-model`**:
  - `AnthropicGateway::model_limits()` 实现:per-model 查表(opus-4-7 / sonnet-4-6 / haiku-4-5),未知 id 回保守默认 + warn
  - `OpenAiCompatGateway::model_limits()` 实现:读 `OpenAiCompatProviderConfig.context_window_tokens` / `max_output_tokens`(新增字段,有默认)
- **`cogito-config`**:
  - `ProviderConfig::OpenAiCompat` 加 `context_window_tokens: u64`(default 32_768)+ `max_output_tokens: u64`(default 4_096)两个字段
...
pub trait ModelGateway: Send + Sync {
    ...
    /// conservative values; provider adapters MUST override.
...
**Provider 实现责任**:
- `AnthropicGateway`:per-model 查表(opus-4-7 → 1M / sonnet-4-6 → 200k / haiku-4-5 → 200k);未知 model id 用保守默认 + tracing::warn
- `OpenAiCompatGateway`:从 `OpenAiCompatProviderConfig.context_window_tokens` / `max_output_tokens` 读(用户配置,因为 vLLM/SGLang 是用户自己部署的)
- `MockModelGateway`(`cogito-test-fixtures`):默认实现即可,测试用保守值
...
pub enum TokenThreshold {
    ...
    Ratio {
        ...
        safety_headroom: u64,       // 留给当前 turn 的预留 token
    },
...
impl Default for TokenThreshold {
    fn default() -> Self {
        // 0.75 = 留 25% 给当前 turn(input + output);8192 头部安全垫吸收估算偏差
...
async fn maybe_compact(&self, input: CompactionInput<'_>) -> Result<Vec<CompactionApplied>, ContextError> {
    ...
    let max_tokens = match &self.config.max_tokens {
        ...
        TokenThreshold::Ratio { of_context_window, safety_headroom } => {
            ...
            // 留出 output token 预留 + 用户显式 safety_headroom,两者取较大
            let total_headroom = (*safety_headroom).max(limits.max_output_tokens);
            raw.saturating_sub(total_headroom)
        }
    };
...
| 8 | `max_tokens: Ratio{0.75, 8192}`,同 strategy 切 model:Opus(1M)→ ~742k 阈值;Sonnet(200k)→ ~142k 阈值;vLLM 32k(ProviderConfig 声明)→ ~16k 阈值 | 阈值自动跟随 `model_gateway.model_limits()`,strategy 不动 |
...
| ModelGateway::model_limits | `crates/cogito-model/tests/limits.rs` | AnthropicGateway 查表(opus / sonnet / haiku)+ 未知 id fallback;OpenAiCompatGateway 读 ProviderConfig 字段;MockModelGateway 默认值 |
...
- **Sprint 7 SkillsInjector**:计算注入 SKILL.md 内容后是否超 `max_output_tokens`,超则只注 description 摘要
...
- **v0.2 H05 surface sizing**:tool schema 总字节数过大时按 `model_limits` 收窄
- **v0.5 多模态**:`ContentBlock::Image` 通过 `model_limits` 的(未来扩展)`max_image_tokens` / `max_image_dimensions` 决定是否压缩
...
**缓解**:
- 默认 `Ratio { 0.75, 8192 }` 保留 25% headroom + 8192 额外安全垫(Compactor 计算时 `safety_headroom.max(max_output_tokens)`,自动按模型 max_output 加大)
- 优先使用 `last_usage.prompt_tokens`(真实值);只在它缺失时才退到 char 估算
- 文档明示估算精度;用户可改 `safety_headroom` 或切到 `Absolute` 自定值
...
[cogito-model:     [cogito-config:    [StepRecorder 校验
 model_limits       OpenAiCompat       + 4 个 record_*]
 lookup tables]     +context_window]
        │                │                  │
        └────────┬───────┴──────────────────┘
                 ▼
