...
- **`cogito-protocol`** 新增类型(全部 additive,不触发 `SCHEMA_VERSION` bump,符合 ADR-0007 §"forward-compat"):
  ...
  - **`ModelGateway::model_limits() -> ModelLimits`** 方法(additive,带保守默认实现);`ModelLimits { model_id, context_window_tokens }` 类型——使 Compactor 阈值跨 provider 自适应,详 §6.7 / §10
  - **`parse_context_window_suffix(model_id) -> Option<u64>`** 公共 helper:统一解析 `[<size>]` 后缀约定(`opus-4-7[1m]` → 1_000_000;`Llama-3.3-70B[32k]` → 32_000),供各 gateway 共享,详 §6.7.1
  - `TokenThreshold` enum(`Ratio` / `Absolute`)作为 `TruncateConfig.max_tokens` 字段类型,详 §10.1
- **`cogito-model`**:
  - `AnthropicGateway::model_limits()` 实现:优先解析 model_id 后缀;无后缀按 base 查表(opus-4-7 / sonnet-4-6 / haiku-4-5,均 200k 默认);未知 id 回保守默认 + warn。发送给 API 的 model_id 包含后缀(Anthropic 接受 `claude-opus-4-7[1m]` 字面值)
  - `OpenAiCompatGateway::model_limits()` 实现:优先解析后缀;无后缀回 `OpenAiCompatProviderConfig.context_window_tokens`(Optional);均无则 32_768 + warn。发送给 vLLM/SGLang 的 model_id **strip 后缀**(`"Llama-3.3-70B[32k]"` → 发送 `"Llama-3.3-70B"`)
- **`cogito-config`**:
  - `ProviderConfig::OpenAiCompat` 加 `context_window_tokens: Option<u64>` 字段(后缀缺失时的 fallback;default None → gateway 兜底 32_768)。**不加** `max_output_tokens` 字段——本轮输出上限由 `strategy.model_params.max_tokens` 决定(strategy 维度,既有字段不动)
...
- Token 估算精度提升(provider tokenizer 抽象 / `ModelGateway::count_tokens`):v0.1 不做,Compactor 用 `last_usage.prompt_tokens`(真实)或 char/4 估算;v0.2 加 summarize Compactor 时再评估是否值得拖入 tiktoken / cl100k 等编码表大依赖(见 §15.1 R1)
...
### 6.7 `ModelGateway::model_limits` 扩展(跨 provider 自适应阈值的契约)

Sprint 6 在 `ModelGateway` trait 加一个 additive 方法,使 Compactor 能根据当前 session 使用的 model 自适应触发阈值——避免 Opus(1M)/ Sonnet(200k)/ vLLM 私有部署(32k)用同一硬编码 100k 的 footgun。

```rust
// cogito-protocol::gateway
#[async_trait]
pub trait ModelGateway: Send + Sync {
    async fn stream(&self, input: ModelInput, ctx: ExecCtx) -> Result<BoxStream<'static, Result<ModelEvent, ModelError>>, ModelError>;
    fn provider_id(&self) -> &'static str;

    /// Limits of the model this gateway serves. Used by Compactor for adaptive
    /// thresholds and (future) H05 for surface sizing. Default impl returns
    /// conservative values; provider adapters SHOULD override.
...
**为什么 `ModelLimits` 不含 `max_output_tokens`**:输出上限是 strategy 维度的决定("我这一轮想要多大输出"),不是 model fact。strategy 已有 `model_params.max_tokens: Option<u32>` 字段作为发给 API 的 max_tokens——Sprint 6 不动它。Compactor 计算阈值只需 `context_window_tokens` + 用户配置的 `safety_headroom`(见 §10.1)。

**Provider 实现责任**(详 §6.7.1 后缀约定):
- `AnthropicGateway`:解析 model_id 后缀(若有)→ 否则按 base 查表(均 200k 默认)→ 否则保守默认 200k
- `OpenAiCompatGateway`:解析后缀 → 否则读 `OpenAiCompatProviderConfig.context_window_tokens`(Option)→ 否则 32_768 + warn
- `MockModelGateway`(`cogito-test-fixtures`):用 default impl 即可;特定测试用 builder 注入自定义 limits

**为何 default impl 而非强制 override**:既有测试 fixture / consumer mock 不需要立即改;只是会用 32k 保守阈值。强制 override 会强迫所有 ModelGateway 实现者立即变更,违反 additive 原则。

**为何不放在 Strategy 上**:context window 是 model 维度的事实,不是 strategy 维度的选择。把它放 Strategy 会让"切 model 时忘改阈值"成为 footgun(rebalance §1 已强调团队成员入门曲线问题)。

**复用范围**:`ModelLimits` 在 Sprint 6 仅由 Compactor 使用;v0.1 后续 sprint 可复用——Sprint 9 multi-model 选 strategy 时按 limits 排序、v0.2 H05 surface sizing 按 limits 收窄、v0.5 多模态摘要按 limits 决定分辨率。

### 6.7.1 Model id `[<size>]` 后缀约定

Sprint 6 引入统一约定:**model id 可携带 `[<size>]` 后缀声明 context window**。`<size>` 形如 `1m` / `200k` / `32k` / `8192`(纯数字):

- `claude-opus-4-7[1m]` → 1_000_000
- `claude-sonnet-4-6[200k]` → 200_000
- `Llama-3.3-70B-Instruct[32k]` → 32_000
- `gpt-4o[128000]` → 128_000(纯数字,无单位)
...
pub fn parse_context_window_suffix(model_id: &str) -> Option<u64> {
    let re = regex::Regex::new(r"\[(\d+)([kKmM])?\]$").ok()?;
...
**Anthropic 与 OpenAi-compat 的关键差异——后缀的去向**:

| Provider | 后缀对 API 的语义 | gateway 行为 |
|---|---|---|
| **Anthropic** | `claude-opus-4-7[1m]` 是 Anthropic API 接受的**真实 model id**(后缀就是 API 字面值的一部分) | 发送给 API 的 model_id **保留后缀**;本地解析后缀填 `model_limits` |
| **OpenAi-compat (vLLM / SGLang)** | vLLM 服务器认 HuggingFace 风格 id(`meta-llama/Llama-3.3-70B-Instruct`),不识别 `[32k]` | 发送给 API 的 model_id **strip 后缀**(`api_model_id()` helper);本地解析填 `model_limits` |

`OpenAiCompatGateway::api_model_id()`(私有 helper):

```rust
fn api_model_id(&self) -> &str {
    self.model_id.split_once('[').map(|(b, _)| b).unwrap_or(&self.model_id)
}
```

**约定的好处**:
- 单一字符串携带"model + window"两个事实——strategy.yaml 不需要 strategy 维度的 `context_window_tokens` 字段
- 切 model 时阈值自动跟随——`model: claude-opus-4-7[1m]` 改成 `model: claude-sonnet-4-6`,truncate 阈值从 750k 自动降到 142k,strategy 其它配置不动
- Anthropic 的 `[1m]` 选择和 vLLM 的 `[32k]` 声明在 cogito 层用同一套语法,降低团队成员心智负担
...
pub enum TokenThreshold {
    /// Adaptive:按当前 ModelGateway::model_limits().context_window_tokens
    /// 的比例触发。跨 provider 通用——切 model 自动跟随。
    Ratio {
        of_context_window: f32,     // 0.0..1.0
        safety_headroom: u64,       // 留给本轮 input 增量 + output 的预留 token
    },
    /// Absolute:用户显式锁死,忽略 model_limits。
    /// 用于"我就是要在这个数压缩"的特殊场景(如 cost cap)。
    Absolute(u64),
}

impl Default for TokenThreshold {
    fn default() -> Self {
        // 0.75 = 留 25% 给本轮 input 增量 + output;8192 安全垫吸收估算偏差。
        // 典型 4-8k output 模型够用;长输出场景(16k+)用户自己调高 safety_headroom。
        Self::Ratio { of_context_window: 0.75, safety_headroom: 8192 }
    }
}
```

**为什么默认 Ratio 而非 Absolute**:strategy 切 model 时,Ratio 自动跟随——Opus(1M)阈值变成 ~742k,Sonnet(200k)变成 ~142k,vLLM 32k 变成 ~16k。用户不需要每次切 model 都回头改 max_tokens。

**Absolute 的合法用例**:
- Cost cap("我不想让单轮 prompt 超过 50k token,无论模型支不支持")
- 测试场景需要精确控制阈值
- 已知 model_limits 不准时的临时覆盖
...
async fn maybe_compact(&self, input: CompactionInput<'_>) -> Result<Vec<CompactionApplied>, ContextError> {
    ...
    let max_tokens = match &self.config.max_tokens {
        ...
        TokenThreshold::Ratio { of_context_window, safety_headroom } => {
            ...
            let raw = (limits.context_window_tokens as f64 * (*of_context_window as f64)) as u64;
    ...
    // ── 步骤 6:在可丢区间内定位首尾 uncovered turn
    ...
    // ── 步骤 7:写 ContextCompacted(StepRecorder 校验 §5.5 invariants)
...
| 8 | `max_tokens: Ratio{0.75, 8192}`,同 strategy 切 model id:`claude-opus-4-7[1m]` → 阈值 ~742k;`claude-sonnet-4-6` → 阈值 ~142k;`Llama-3.3-70B[32k]` → 阈值 ~16k | 阈值自动跟随 `model_gateway.model_limits()`,strategy 其它配置不动 |
...
| 测试 | 文件 | 覆盖 |
|---|---|---|
| TruncateCompactor 全路径 | `tests/truncate_compaction.rs` | §10.4 边界 1-7 + idempotency |
| TruncateCompactor 跨 provider 阈值 | `tests/truncate_threshold_ratio.rs` | §10.4 边界 8:用 MockModelGateway 注入不同 `ModelLimits`(1M / 200k / 32k),断言 `Ratio{0.75, 8192}` 计算出的阈值正确(对照 `Absolute` 路径) |
...
| `parse_context_window_suffix` | `crates/cogito-protocol/tests/model_id_suffix.rs` | `opus-4-7[1m]` → 1M;`Llama[32k]` → 32k;`gpt-4o[128000]` → 128000;无后缀 → None;畸形 `[bad]` → None |
| ModelGateway::model_limits | `crates/cogito-model/tests/limits.rs` | AnthropicGateway:后缀优先 + 无后缀查表 + 未知 id fallback;OpenAiCompatGateway:后缀优先 + ProviderConfig fallback + 双缺失兜底;`api_model_id()` strip 后缀;MockModelGateway 默认值 |
...
### 14.6 `ModelLimits` 的后续复用点

`ModelLimits` 在 Sprint 6 仅由 TruncateCompactor 使用,但抽象设计已涵盖更广场景——后续无需 amend trait:

- **Sprint 7 SkillsInjector**:计算注入 SKILL.md 内容后剩余 `context_window_tokens - last_usage.prompt_tokens`,不够则只注 description 摘要
- **Sprint 9 multi-model strategy selector**:strategy.yaml 加 `min_context_window` 字段,启动时按 `model_limits` 过滤掉容量不够的 strategy
- **v0.2 Summary Compactor**:`Ratio` 阈值 + `summarization_model` 各自独立 `model_limits` 决策——摘要模型可以用更小窗口的 cheaper 模型
- **v0.2 H05 surface sizing**:tool schema 总字节数过大时按 `context_window_tokens` 收窄
- **v0.5 多模态**:`ModelLimits` additive 加(未来)`max_image_tokens` / `max_image_dimensions` 字段决定 `ContentBlock::Image` 是否压缩
...
- 阈值算出 142_000(Sonnet × 0.75 - 8192)→ 估算到 150_000(实际 130_000)→ 不必要的压缩
- 反之,估算到 130_000(实际 150_000)→ 该压缩没压,模型报 length error

**缓解**:
- 默认 `Ratio { 0.75, 8192 }` 保留 25% headroom + 8192 安全垫(典型 4-8k output 模型够用)
- 长输出场景(16k+ 报告生成)用户自己把 `safety_headroom` 拉高,或切到 `Absolute` 精确锁定
- 优先使用 `last_usage.prompt_tokens`(真实值);只在它缺失时才退到 char 估算
- 文档明示估算精度

非阻塞——v0.2 加 summarize 时同样问题,届时考虑统一抽象(可能在 ModelGateway 加 `count_tokens` 方法,返回精确 token 数;default impl 用 char/4)。
...
[ADR-0008 起草]
        ...
        ▼
[cogito-protocol 新增]──────────────────────┐
  ├─ 4 traits + 4 events + 4 configs        │
  ├─ ModelGateway::model_limits + ModelLimits│
  ├─ parse_context_window_suffix helper     │
  └─ TokenThreshold enum                    │
        │                                   │
        ├────────────────┐                  │
        ▼                ▼                  ▼
[cogito-model:     [cogito-config:    [StepRecorder 校验
 Anthropic/OAi-     OpenAiCompat       + 4 个 record_*]
 compat 用后缀+     +context_window
 fallback 实现       (Option fallback)]
 model_limits]
        │                │                  │
        └────────┬───────┴──────────────────┘
                 ▼
[cogito-context: build_pipeline + no-op + truncate(用 model_limits)]
                 │
                 ▼
...
ADR-0008 与 cogito-protocol 新增高度并行(ADR 描述什么,protocol 落地什么)。cogito-model / cogito-config 节点彼此独立,可以并行;两者都依赖 protocol 节点完成。其余按图依赖。
