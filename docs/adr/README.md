...
- [0034](./0034-runtime-session-registry-lifecycle.md) — Runtime session-registry lifecycle: add public `Runtime::get_session(id)` (live-handle lookup) + `Runtime::close_session(id, deadline)` (drive shutdown, then deregister) so a session can be re-`Resume`d within the *same* `Runtime` (the insert-only `sessions` DashMap makes reopen impossible today); driven by praxis RR-7 / issue #55; pure Runtime surface, no protocol change — **Proposed (v0.4 SaaS-ready; not blocking v0.3)**
