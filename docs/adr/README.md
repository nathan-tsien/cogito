...
- 0011 — Subagent execution model — **split** by 2026-05-22 rebalance: v0.2 Sprint 11 ships minimal `delegate(role, input) → output` tool (lives in `cogito-core::runtime::subagent`); v0.3 amendment adds full `BrainSpawner` trait + 4 tools + parent-child event tree + crash semantics
