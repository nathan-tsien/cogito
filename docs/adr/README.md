...
- [0038](./0038-loop-iteration-budget.md) — Agent-loop iteration budget: enforce the already-declared-but-dead `HarnessStrategy::max_turns` (default 16) by adding `TurnFailureReason::MaxTurnsExceeded { turns }` (additive, no `SCHEMA_VERSION` bump) and bounding the H01 inner loop by **model-call count** (replay-derivable per AGENTS.md §3). On-hit = **fail** (honest primitive; continue/summarize are consumer policy layered on the failure); orthogonal to `MAX_CONSECUTIVE_TOOL_ERRORS` and `TurnTimedOut` — **Accepted (2026-06-07), implemented (PR #66)**
- [0039](./0039-hitl-over-suspension-seam.md) — Human-in-the-loop is a **consumer flow over the suspension seam**, not a core feature: ask-user (Manus `message_ask_user`) and tool approval gates both reduce to the existing `InvokeOutcome::Async` → `Paused`/`TurnPaused` → resume-on-`JobCompletion` path ("humans-as-jobs"); core adds no HITL tool/UI/policy. Specifies the **durable-`JobManager` contract** a SaaS consumer must satisfy (bundled `LocalJobManager` is in-process / CLI-grade and strands a parked turn across restart) + one additive observation-only `JobStatus::AwaitingInput`. Boundary mirrors ADR-0014/0037 — **Accepted (2026-06-07), `AwaitingInput` implemented**
- [0040](./0040-turn-level-truncation-signal.md) — Turn-level truncation signal: a `MaxTokens`-truncated turn was indistinguishable from a clean completion at the turn boundary (`output.stop_reason` dropped in `model_completed::transit`; no `StreamEvent` carries it live). Surface it by carrying `stop_reason: Option<StopReason>` on the **broadcast** `StreamEvent::TurnCompleted` (additive, serde-default, like `subagent_call_id`) + a `tracing::warn!` on truncation. No new `TurnOutcome`/`TurnFailureReason` variant (truncation is a completion caveat, not a failure; partial text stays in `final_assistant_content`); persisted event unchanged (truth already in adjacent `ModelCallCompleted`). Strategy policy (fail/auto-continue) + replay-parity persistence DEFERRED. Consumer RR (praxis RR-8), issue #69 part 1 — **Accepted (2026-06-11), implemented**
